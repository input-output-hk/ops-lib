# Adapted from https://github.com/NixOS/nixpkgs/blob/33d12d781e245092f19b78fc701ffef6263cfe0c/nixos/modules/services/databases/clickhouse.nix
{ config, lib, pkgs, ... }:
let
  cfg = config.services.clickhouse-custom;
in
with lib;
{
  ...
  options = {
    ...
    services.clickhouse-custom = {
      ...
      extraCfg = mkOption {
        ...
        default = "";
        description = ''
          Extra XML config for Clickhouse server.
        '';
      };
    };
  ...
  config =
    let
      clickhouseServerCfg = pkgs.writeText "config.xml" ''
        <?xml version="1.0"?>
        <!--
        ...
        <yandex>
            ...
            <users_config>${pkgs.clickhouse}/etc/clickhouse-server/users.xml</users_config>
    ...
    in mkIf cfg.enable {
      ...
      systemd.services.clickhouse-custom = {
